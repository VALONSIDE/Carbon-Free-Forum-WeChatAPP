...
Page({
  ...
  onLoad: function(options) {
    // 获取帖子ID并添加错误处理
    console.log('页面加载，接收到的options:', options);
    
    if (!options || !options.id) {
      console.log('未接收到帖子ID，使用默认ID: 1');
      this.getPostData(1);
      return;
    }
    ...
    if (isNaN(postId)) {
      console.error('无效的帖子ID:', options.id);
      this.getPostData(1);
      return;
    }
  ...
  getPostData: function(postId) {
    console.log('开始获取帖子数据，ID:', postId);
    
    // 确保postId是数字类型
    const postIdNum = Number(postId);
    console.log('转换后的postId:', postIdNum);
    
    // 使用数字类型的postId查找帖子
    const postData = this.data.postsDatabase.find(post => post.id === postIdNum);
    
    if (postData) {
      console.log('找到匹配的帖子:', postData.title);
      this.setData({
        post: postData
      }, () => {
        console.log('帖子数据设置完成:', this.data.post.title);
      });
    } else {
      console.log('未找到匹配的帖子，使用默认帖子');
      this.setData({
        post: this.data.postsDatabase[0]
      }, () => {
        console.log('默认帖子数据设置完成:', this.data.post.title);
      });
    }
  },
